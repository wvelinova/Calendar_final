...
$lastDay = date('w', strtotime($y.'-'.$m.'-'.$d)); // Първият ден от месеца какъв ден от седмицата е
$lastDay--; // Изваждаме 1, за да започва от понеделник
if($lastDay == -1)
{
...
// Допълва първите дни на следващия месец:
if($lastDay < 6)
{
    $fr = 1;
    for($i = $lastDay; $i<6; $i++)
...
$delArray = array_chunk($Cells, 7); // Раделяме масива на двумерен от по 7 елемента
?>
<!doctype html>
<html lang="en">
  <head>
          ...
          <table class="table table-bordered text-center">
            ...
            <tbody>
              ...
              <?php
               ...
               $countWeek = 0; // Брояч на седмиците в месеца
               $countDay = 0; // Брояч на клетките в седмицата 
                foreach ($delArray as $week) // Обхождаме седмиците от месеца 
                {
                    
                    echo '<tr>'; // Отваряме ред
                    $countWeek++; // увеличаваме брояча за седмици
                    foreach($week as $cell) //обхождаме дните от седмицата
                    {
                        
                       $countDay++; // увеличаваме брояча за клетките 
                // по-светлите дни, които са от предишен и следващ месец
                if($countWeek == 1 || $countWeek == count($delArray))// условие за определяне на първата и последната седмица 
                   {
                     if($countDay<=$dayOfWeek || $countDay > $d+$dayOfWeek ) // условие за определяне на дните от предишния и следващия месец
                      {
                       ?>
                       <td class="text-black-50"><?php echo $cell; ?></td>
                       <?php
                      }
                    else {
                        ?> 
                        <td class="fw-bold"><?php echo $cell; ?></td> 
